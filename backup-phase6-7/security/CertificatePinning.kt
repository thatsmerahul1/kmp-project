...
class SecureCertificatePinningManager(
        ...
        return try {
            ...
            if (isValid) {
                ...
                if (pin.reportOnly) {
                    ...
                    Result.Error(SecurityException("Certificate pinning validation failed for $hostname"))
                }
            }
            ...
            Result.Error(SecurityException("Certificate validation failed: ${e.message}", e))
        }
    }
    ...
    override suspend fun addPin(hostname: String, pin: CertificatePin): Result<Unit> {
        return try {
            if (pin.pins.isEmpty()) {
                return Result.Error(SecurityException("Certificate pin cannot be empty"))
            }
            
            if (pin.enforceBackupPin && pin.pins.size < 2) {
                return Result.Error(SecurityException("Backup pin required but only one pin provided"))
            }
            ...
            Result.Error(SecurityException("Failed to add certificate pin: ${e.message}", e))
        }
    }
    ...
    override suspend fun removePin(hostname: String): Result<Unit> {
        return try {
            ...
            Result.Error(SecurityException("Failed to remove certificate pin: ${e.message}", e))
        }
    }
    ...
    override suspend fun rotatePins(hostname: String, newPins: List<CertificatePin>): Result<Unit> {
        return try {
            if (newPins.isEmpty()) {
                return Result.Error(SecurityException("New pins cannot be empty"))
            }
            
            val oldPin = pinCache[hostname]
            val newPin = newPins.first()
            
            // Validate new pin configuration
            if (newPin.enforceBackupPin && newPin.pins.size < 2) {
                return Result.Error(SecurityException("Pin rotation requires backup pin"))
            }
            ...
            Result.Error(SecurityException("Pin rotation failed: ${e.message}", e))
        }
    }
    ...
    override suspend fun validatePinConfiguration(): Result<Boolean> {
        return try {
            ...
            Result.Error(SecurityException("Pin configuration validation failed: ${e.message}", e))
        }
    }
    ...
    private fun handleNoPinConfigured(hostname: String): Result<Boolean> {
        return when (config.fallbackBehavior) {
            FallbackBehavior.BLOCK_CONNECTION -> {
                Result.Error(SecurityException("No certificate pin configured for $hostname"))
            }
