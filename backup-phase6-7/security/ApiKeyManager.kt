...
class SecureApiKeyManager(
    ...
    override suspend fun getDecryptedApiKey(service: ApiService): Result<String> = mutex.withLock {
        try {
            ...
            val encryptedKey = secureStorage.retrieve("api_key_${service.identifier}")
                ?: return Result.Error(SecurityException("API key not found for service: ${service.identifier}"))
            
            // Decrypt the API key
            val decryptedKey = encryptionProvider.decrypt(encryptedKey)
                ?: return Result.Error(SecurityException("Failed to decrypt API key for service: ${service.identifier}"))
            
            // Validate integrity if enabled
            if (config.enableIntegrityChecks) {
                val isValid = validateKeyIntegrity(service, decryptedKey)
                if (!isValid) {
                    return Result.Error(SecurityException("API key integrity check failed for service: ${service.identifier}"))
                }
            }
            ...
            Result.Error(SecurityException("API key retrieval failed: ${e.message}", e))
        }
    }
    ...
    override suspend fun storeEncryptedApiKey(service: ApiService, apiKey: String): Result<Unit> = mutex.withLock {
        try {
            ...
            if (!isValidApiKeyFormat(apiKey)) {
                return Result.Error(SecurityException("Invalid API key format for service: ${service.identifier}"))
            }
            
            // Encrypt the API key
            val encryptedKey = encryptionProvider.encrypt(apiKey)
                ?: return Result.Error(SecurityException("Failed to encrypt API key for service: ${service.identifier}"))
            
            // Store encrypted key
            val stored = secureStorage.store("api_key_${service.identifier}", encryptedKey)
            if (!stored) {
                return Result.Error(SecurityException("Failed to store encrypted API key for service: ${service.identifier}"))
            }
            ...
            Result.Error(SecurityException("API key storage failed: ${e.message}", e))
        }
    }
    ...
    override suspend fun rotateApiKey(service: ApiService): Result<String> {
        try {
            ...
            when (val storeResult = storeEncryptedApiKey(service, newApiKey)) {
                is Result.Success -> {
                ...
                is Result.Error -> {
                    return Result.Error(storeResult.exception)
                }
            }
            
        } catch (e: Exception) {
            return Result.Error(SecurityException("API key rotation failed: ${e.message}", e))
        }
    }
    ...
    override suspend fun validateApiKeyIntegrity(service: ApiService): Result<Boolean> {
        try {
            ...
            val storedHash = secureStorage.retrieve("api_key_integrity_${service.identifier}")
                ?: return Result.Error(SecurityException("Integrity hash not found for service: ${service.identifier}"))
            ...
            Result.Error(SecurityException("Integrity validation failed: ${e.message}", e))
        }
    }
...
private data class CachedKey(
    ...
    fun isExpired(): Boolean {
        ...
        val ttlDuration = kotlinx.datetime.DateTimePeriod(hours = ttlHours.toInt())
...
/**
 * Security exception for API key management
 */
