...
class WeatherKMPMetricsCollector(
    ...
    override suspend fun startCollection(): Result<Unit> {
        return try {
            if (collectionJob?.isActive == true) {
                return Result.Error(DomainException.Telemetry.InitializationFailed("Metrics collection already running"))
            }
            ...
            Result.Error(DomainException.Telemetry.InitializationFailed("Failed to start metrics collection: ${e.message}"))
        }
    }
    ...
    override suspend fun stopCollection(): Result<Unit> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.InitializationFailed("Failed to stop metrics collection: ${e.message}"))
        }
    }
    ...
    override suspend fun collectSnapshot(): Result<MetricsSnapshot> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed("Failed to collect metrics snapshot: ${e.message}"))
        }
    }
    ...
    override suspend fun recordCustomMetric(
        ...
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed("Failed to record custom metric: ${e.message}"))
        }
    }
