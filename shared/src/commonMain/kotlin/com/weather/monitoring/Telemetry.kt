...
class WeatherKMPTelemetryProvider : TelemetryProvider {
    ...
    override suspend fun recordMetric(metric: TelemetryMetric): Result<Unit> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed(metric.name))
        }
    }
    ...
    override suspend fun startSpan(span: TelemetrySpan): Result<SpanContext> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.SpanOperationFailed("start: ${e.message}"))
        }
    }
    ...
    override suspend fun endSpan(context: SpanContext, result: SpanResult): Result<Unit> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.SpanOperationFailed("end: ${e.message}"))
        }
    }
    ...
    override suspend fun recordEvent(event: TelemetryEvent): Result<Unit> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed(event.name))
        }
    }
    ...
    override suspend fun flush(): Result<Unit> {
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed("flush: ${e.message}"))
        }
    }
...
object TelemetryUtils {
    ...
    suspend fun recordApiCall(
        ...
        return try {
            metrics.forEach { metric ->
                when (val result = telemetryProvider.recordMetric(metric)) {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed("api_call"))
        }
    }
    ...
    suspend fun recordError(
        ...
        return try {
            ...
            Result.Error(DomainException.Telemetry.MetricRecordingFailed("error"))
        }
    }
    ...
    suspend fun <T> withSpan(
        ...
        val context = when (val result = telemetryProvider.startSpan(span)) {
            ...
            is Result.Loading -> throw DomainException.Telemetry.SpanOperationFailed("Unexpected loading state")
        }
